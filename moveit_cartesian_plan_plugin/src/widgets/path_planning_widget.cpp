...
void PathPlanningWidget::retransformPoints(const visualization_msgs::InteractiveMarkerFeedbackConstPtr feedback)
{ 
  // Current object pose
  geometry_msgs::Pose current_object_pose = feedback->pose;
  ...
  // Get object transform from feedback
  object_world_tfmsg.translation.x = current_object_pose.position.x;
  object_world_tfmsg.translation.y = current_object_pose.position.y;
  object_world_tfmsg.translation.z = current_object_pose.position.z;
  object_world_tfmsg.rotation = current_object_pose.orientation;
  tf::transformMsgToTF(object_world_tfmsg, object_world_tf);
  ...
  std::string mesh_name = ui_.mesh_name_lbl->text().toStdString();
  Q_EMIT modifyMarkerControl_signal(mesh_name, current_object_pose);
